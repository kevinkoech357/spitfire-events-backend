...
def add_user_to_group(group_id, new_member_id):
    ...
    try:
        ...
        if group is None or new_member is None:
            return jsonify(
                {"message": f"Group or user not found", "error": "Not Found"}
            ), 404

        newgroup = new_member.user_groups
        if group.id in [group.id for group in newgroup]:
            return jsonify(
                {"error": "Forbidden", "message": "User already in group"}
            ), 403
        ...
        return jsonify({"data": group.id, "message": "User added to Group"}), 201
...
def update_group(group_id):
    ...
    try:
        ...
        if not group:
            return jsonify(
                {"error": "Not Found", "message": f"Group with ID {group_id} not found"}
            ), 404
        ...
        return jsonify(
            {"error": "Bad Request", "message": " Something went wrong"}
        ), 400
...
def remove_user_from_group(group_id, member_id):
    ...
    try:
        ...
        if group is None or member is None:
            return jsonify(
                {"message": "Group or user not found", "error": "Not Found"}
            ), 404

        user_groups = member.user_groups
        if group_id not in [group.id for group in user_groups]:
            return jsonify(
                {"error": "forbidden", "message": "User is not a member of this group"}
            ), 403
        ...
        return jsonify(
            {"message": "User removed from group successfully", "data": group_id}
        ), 204
...
def create_group():
    ...
    # Handle exceptions and return an error response if any occur.
        ...
        return jsonify(
            {"message": "group creation failed", "error": "Bad Request"}
        ), 400
